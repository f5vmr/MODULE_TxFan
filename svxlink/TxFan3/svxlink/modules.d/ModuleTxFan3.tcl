...
namespace eval TxFan3 {
	...
	proc main {} {
		...
		switch $CFG_MODE {
			...
			COUNT_DOWN {
				set Heatsink_temperature [expr [expr [expr [expr [expr [exec cat /sys/bus/iio/devices/iio\:device0/in_voltage2_raw] * 4.88] / 10] - 50] * 1.8] + 32]
				if {$Repeater_Online == 1 && $Heatsink_temperature > $CFG_Heatsink_Critical_Setpoint} {
					...
					rptrOff
						...
						printInfo "Exciter Transmitter: [format "%.1f" $Heatsink_temperature]"
						set fp [open $CFG_FAN_GPIO w]
						puts $fp "1"
						close $fp
						set timer $CFG_DELAY
					...
					if {$timer == 0} {
						...
						printInfo "Exciter Transmitter Fan Timer: $timer"
